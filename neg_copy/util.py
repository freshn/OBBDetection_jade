...
def sampling_new_bbox_center_point2(img_shape):
    #### sampling space ####
    height, width, nc = img_shape
    ### left top ###
    '''
    ...
    '''
    search_x_left, search_y_left, search_x_right, search_y_right = width * 0.1 , height * 0.3 , \
                                                                   width * 0.9 , height * 0.95

    return [int(search_x_left), int(search_y_left), int(search_x_right), int(search_y_right)]
...
def random_add_patches2(bbox_img, rescale_boxes, shape, paste_number, iou_thresh):
    ...
    center_search_space = sampling_new_bbox_center_point2(shape)  # 选取生成随机点区域
    success_num = 0
    new_bboxes = []
    cl = 1
    while success_num < paste_number:
...
def random_positions(bbox_img, shape, paste_number=1):
    bbox_h, bbox_w, bbox_c = bbox_img
    img_h,img_w,img_c = shape
    center_search_space = sampling_new_bbox_center_point2(shape)  # 选取生成随机点区域
    success_num = 0
