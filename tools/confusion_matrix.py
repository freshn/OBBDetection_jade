...
def calculate_confusion_matrix(dataset,
    ...
    for idx, per_img_res in enumerate(results):
        if isinstance(per_img_res, tuple):
            ...
            res_bboxes = per_img_res  # obb formation [xc,yc,w,h,theta,confidence]
        ann = dataset.get_ann_info(idx)
        gt_bboxes = ann['bboxes']  # poly formation [x1,y1,x2,y2,x3,y3,x4,y4]
        labels = ann['labels']
...
def analyze_per_img_dets(confusion_matrix,
    ...
    for det_label, det_bboxes in enumerate(result):
        if nms_iou_thr:
            det_bboxes, _ = bt.bbox_nms(
                det_bboxes[:, :5],
                det_bboxes[:, -1],
                nms_iou_thr,
                score_threshold=score_thr)
        ious = bt.bbox_overlaps(det_bboxes[:, :5], gt_bboxes)        
