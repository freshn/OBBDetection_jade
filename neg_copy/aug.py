...
def copybackground(image_dir, save_base_dir, small_img_dir):
    image = cv2.imread(image_dir)
    if len(image) == 0:
        return
    all_boxes = []
    ...
    for small_img_dirs in small_img_dir:
        image_bbox = cv2.imread(small_img_dirs)
        # roi = image_bbox
        roi = suo_fang(image_bbox,area_max=440,area_min=16)

        new_bboxes = random_positions(roi.shape, image.shape, paste_number=1)
        count = 0
        for new_bbox in new_bboxes:
            count += 1
            ...
            try:
                ...
                # print(roi.shape, image.shape, bbox_top, bbox_bottom, bbox_left, bbox_right)
                image[bbox_top:bbox_bottom, bbox_left:bbox_right] = cv2.seamlessClone(roi, image[bbox_top:bbox_bottom, bbox_left:bbox_right],
                                                                                      mask, center, cv2.NORMAL_CLONE)
            except ValueError:
                print("---")
    ...
    # cv2.imwrite(join(save_dir, basename(image_dir).replace('.jpg', '_augment.jpg')), image)
    # print(image_dir)
    cv2.imwrite(join(save_dir, basename(image_dir)), image)
